...
```{r}
#Looking at over time incidents for each of the most prevenlent ones in each category from step 1:
...
ggplot(three_yearly, aes(x = Year, y = Bias, size = incidents, fill = Bias)) +
  ...
  theme_minimal() +
  labs(
    title = "Hate Crime Incidents Over Time",
...
plots <- lapply(offense_list, function(off) {
  ggplot(state_top_offenses %>% filter(Offense == off), aes(x = reorder(State, incidents), y = incidents, fill = Offense)) +
